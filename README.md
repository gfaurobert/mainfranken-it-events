...
npm run dev
npm test
npm run smoke:mcp   # with dev server running
```
...
```bash
curl -s "http://localhost:3000/events?city=Würzburg"
curl -s "http://localhost:3000/events?city=Würzburg&tags=meetup"
curl -s "http://localhost:3000/events?query=kubernetes&is_free=true&limit=10"
```
...
```bash
curl -s "http://localhost:3000/events/<uuid>"
```
...
Connect your MCP client to `http://localhost:3000/mcp` using Streamable HTTP transport.
